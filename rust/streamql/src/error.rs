...
pub(crate) trait StreamQLErrorExt<T> {
    fn required(self, token: &str) -> Result<T>;
}

impl<T> StreamQLErrorExt<T> for Option<T> {
    fn required(self, token: &str) -> Result<T> {
        match self {
            Some(value) => Ok(value),
...
impl<T, E> StreamQLErrorExt<T> for std::result::Result<T, E> {
    fn required(self, token: &str) -> Result<T> {
        match self {
            Ok(value) => Ok(value),
...
impl From<pest::error::Error<crate::parser::Rule>> for StreamQLError {
    fn from(value: pest::error::Error<crate::parser::Rule>) -> Self {
        StreamQLError::LexerError(Box::new(value))
    }
}

impl From<ParserError> for StreamQLError {
    fn from(value: ParserError) -> Self {
        StreamQLError::ParserError(Box::new(value))
    }
}

pub(super) type Result<T> = std::result::Result<T, StreamQLError>;
