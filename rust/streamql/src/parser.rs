...
#[derive(Parser)]
#[grammar = "streamql.pest"]
struct StreamQLParser;
...
fn build_ws_clause_ast(pair: Pair<Rule>) -> Result<ClauseWs> {
    ...
    match field.as_rule() {
        ...
        Rule::WsIntFieldName => match field.as_str() {
            "len" => {
                clause.len = Some(build_expr_int_ast(expr)?);
            }
            t => unknown!("WsIntFieldName.{}", t),
        },
...
pub fn parse(input: &str) -> Result<Query> {
    let mut pairs = StreamQLParser::parse(Rule::StreamQL, input)?;

    let pair = pairs.next().required("StreamQL")?;
    match pair.as_rule() {
        Rule::StreamQL => {
            let pair = pair.into_inner().next().required("StreamQL.query")?;
            build_query_ast(pair)
        }
        t => unknown!("StreamQL.{:?}", t),
    }
}
