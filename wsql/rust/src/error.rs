...
pub(crate) trait WSQLErrorExt<T> {
    fn required(self, token: &str) -> Result<T>;
}

impl<T> WSQLErrorExt<T> for Option<T> {
    fn required(self, token: &str) -> Result<T> {
        match self {
            Some(value) => Ok(value),
...
impl<T, E> WSQLErrorExt<T> for std::result::Result<T, E> {
    fn required(self, token: &str) -> Result<T> {
        match self {
            Ok(value) => Ok(value),
...
impl From<pest::error::Error<crate::parser::Rule>> for WSQLError {
    fn from(value: pest::error::Error<crate::parser::Rule>) -> Self {
        WSQLError::LexerError(Box::new(value))
    }
}

impl From<ParserError> for WSQLError {
    fn from(value: ParserError) -> Self {
        WSQLError::ParserError(Box::new(value))
    }
}

pub(super) type Result<T> = std::result::Result<T, WSQLError>;
