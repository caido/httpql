...
mod tests {
    ...
    fn test_ast(#[case] case: u32) {
        let input = std::fs::read_to_string(format!("../tests/ast/{case}/input.wsql")).unwrap();
        let output = std::fs::read_to_string(format!("../tests/ast/{case}/output.ast")).unwrap();
        let query = WSQL::parse(&input).unwrap();
    ...
    fn test_error(#[case] case: u32) {
        let input = std::fs::read_to_string(format!("../tests/error/{case}/input.wsql")).unwrap();
        assert!(WSQL::parse(&input).is_err());
    }
    ...
    //     let input = std::fs::read_to_string(format!("../tests/regex/{}/input.wsql", case)).unwrap();
    //     let test = serde_json::from_str::<Test>(
    //         &std::fs::read_to_string(format!("../tests/regex/{}/test.json", case)).unwrap(),
    //     )
    //     .unwrap();

    //     let query = WSQL::parse(&input);
    //     match test {
    ...
    //     let input =
    //         std::fs::read_to_string(format!("../tests/string/{}/input.wsql", case)).unwrap();
    //     let test = serde_json::from_str::<Test>(
