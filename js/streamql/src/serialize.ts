...
export const serialize = (query: Query): Result<string, StreamQLError> => {
...
const serializeExprDate = (value: ExprDate): Result<string, StreamQLError> => {
  return ok(`${value.operator.toLowerCase()}:"${value.value}"`);
};

const serializeExprBool = (value: ExprBool): Result<string, StreamQLError> => {
  return ok(`${value.operator.toLowerCase()}:${value.value}`);
};
